...
flags.DEFINE_string(
    "split", 'test',
    "define split for evaluation. will read only split files.")
...
def input_fn_builder(data_dir, vocab_model_file, max_encoder_length,
  ...
  def input_fn(params):
    ...
    split = "train" if is_training else FLAGS.split
    if "tfds://" == data_dir[:7]:
      ...
      input_files = tf.io.gfile.glob(
          os.path.join(data_dir, "fns_summary_{}.tfrecord*".format(split)))
      logger.info('inside load files.....')
      print(os.path.join(data_dir, "fns_summary_{}.tfrecord*".format(split)))
      print(input_files)
    ...
    if is_training:
      d = d.shuffle(buffer_size=3000, reshuffle_each_iteration=True)
      d = d.repeat()
...
#     features = {"document": tf.cast(ids, tf.int32, name="document")}
...
def model_fn_builder_export(transformer_config):
  ...
  def model_fn(features, labels, mode, params):  # pylint: disable=unused-argument
    ...
    if mode == tf.estimator.ModeKeys.TRAIN:
      ...
      eval_metrics = (metric_fn,
                      [tf.expand_dims(total_loss, 0), llh, labels, pred_ids])
      output_spec = tf.compat.v1.estimator.tpu.TPUEstimatorSpec(
          mode=mode,
          loss=total_loss,
    ...
    if mode == tf.estimator.ModeKeys.PREDICT:
      ...
      prediction_dict.update({'document':features})
      prediction_dict.update({'summary':labels})
      output_spec = tf.compat.v1.estimator.tpu.TPUEstimatorSpec(
          mode=mode,
...
def main(_):
  ...
  model_fn = model_fn_builder_export(transformer_config)
  ...
  #   tf.compat.v1.reset_default_graph()
    ...
    latest = FLAGS.init_checkpoint # FLAGS.output_dir+"/model.ckpt-4500"
    model_n = int(FLAGS.init_checkpoint.split("-")[-1])
    tokenizer = tft.SentencepieceTokenizer(
        model=tf.io.gfile.GFile(FLAGS.vocab_model_file, "rb").read())

    output_eval_file = os.path.join(
         FLAGS.output_dir, FLAGS.split+"_doc_out_{}.txt".format(model_n))
    output_eval_file2 = os.path.join(
         FLAGS.output_dir, FLAGS.split+"_summary_out_{}.txt".format(model_n))
    output_eval_file3 = os.path.join(
         FLAGS.output_dir, FLAGS.split+"_predictions_out_{}.txt".format(model_n))
    format_line = lambda  seq: ','.join('%d'%m for m in seq )+"\n"
    with tf.io.gfile.GFile(output_eval_file, "w") as writer1,\
         tf.io.gfile.GFile(output_eval_file2, "w") as writer2,\
         tf.io.gfile.GFile(output_eval_file3, "w") as writer3:
        for item in estimator.predict(input_fn=eval_input_fn, checkpoint_path=latest):
            print(item)
            writer1.write(format_line(item['document']))
            writer2.write(format_line(item['summary']))
            writer3.write(format_line(item['pred_ids']))
        
        # sess.run([text_detoken, gold_detoken, pred_summ_detoken], feed_dict={text:item['document'], 
        #                                                                         gold : item['pred_ids'][0],
        #                                                                         pred_summ : item['summary']
        #                                                                         })
        ...
        #             gold_detoken
        # ))
                    #tokenizer.detokenize(item['summary'])))
